...
export async function GET(req, { params }) {
  ...
  const pick = hasKR ? { family: "KoPub Batang", file: "KopubWorldBatang.ttf" } : { family: "Times New Roman", file: "TimesNewRoman.ttf" };

  let fonts = [];
  try {
    fonts = [await loadFontHTTP(url, pick.family, pick.file)];
  } catch (_) {}

  /* LAYOUT */
  const size =
    text.length > 80 ? Math.max(32, Math.floor(baseSize * 0.6)) : text.length > 50 ? Math.max(40, Math.floor(baseSize * 0.75)) : baseSize;
