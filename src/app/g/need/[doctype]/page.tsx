  ...
  return (
    <main>
      ...
      {rows.length === 0 ? (
        ...
        <ol>
          {rows.map((r) => {
            const last = r.last_ref ? new Date(r.last_ref as unknown as string).toISOString().replace("T", " ").slice(0, 19) : "-";

            return (
              <li key={r.sid} style={{ display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap" }}>
                <Link href={`/w/${r.sid}`}>{r.name || r.sid}</Link>
                <small style={{ opacity: 0.7 }}>{r.sid}</small>
      ...
      <nav style={{ marginTop: 16, display: "flex", gap: 8 }}>
        {page > 1 && <Link href={`/need/${doctype}?page=${page - 1}&limit=${limit}`}>이전</Link>}
        {page < pages && <Link href={`/need/${doctype}?page=${page + 1}&limit=${limit}`}>다음</Link>}
      </nav>
